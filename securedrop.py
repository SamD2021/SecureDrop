...
class SecureDrop:
            ...
            for contact_id in received_data["contacts"]:
                ...
                for contact in contacts:
                    if contact["UserID"] == contact_id:
                        print(
                            f"* {self.decrypt_text(filename, contact['Data'])} <{contact_id}>"
                        )
        else:
    ...
    def send_command(self):
        ...
        status, public_key_str = self.send_file_transfer_request(
            contact_email, file_path
        )
        ...
        if status == "approved":
            ...
            recipient_public_key_bytes = base64.b64decode(
            ...
            with open(file_path, "rb") as file:
                ...
                sequence_number = random.randint(
                    0, 1000000
                )  # Random seed for the sequence_number
                while True:
                    chunk = file.read(chunk_size)
                    ...
                    if not response or response.get("status") != "ok":
                        ...
                        print(
                            f"Getting a response with sequence number {sequence_number}"
                        )
    ...
    def request_user_approval(sender_email, file_name, file_size):
        print(
            f"File transfer request from {sender_email}: {file_name} ({file_size} bytes)"
        )
    ...
    def handle_file_transfer_request(self, received_data):
        ...
        if user_response:
            private_key_str, public_key_str, private_key, public_key = (
                keyGen.generate_and_export_keypair()
            )
    ...
    def handle_reconstruct_file_request(self, data):
        ...
        print(
            f"File {file_name} has been successfully reconstructed at {destination_path}."
        )
